...
export const useCanvasPanZoom = (artboardRef: React.RefObject<HTMLDivElement>, canvasRef: React.RefObject<HTMLDivElement>, selectedTool: string | null) => {
  ...
  const handleMouseUp = () => {
    if (isPanning) {
      ...
      if (canvasRef.current) canvasRef.current.style.cursor = selectedTool === "hand" ? "grab" : "default"
    }
  }
  ...
  useEffect(() => {
    if (canvasRef.current) {
      canvasRef.current.style.cursor = selectedTool === "hand" ? (isPanning ? "grabbing" : "grab") : "default"
    }
