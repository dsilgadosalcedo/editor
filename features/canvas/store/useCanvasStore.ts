    ...
    (set, get) => ({
                  ...
                  return {
                    ...
                    cornerRadius: el.cornerRadius
                      ? Math.max(
                          ...
                          Math.min(
                            el.cornerRadius,
                            Math.max(20, Math.round(el.width * finalScaleX)) /
                              2,
                            Math.max(20, Math.round(el.height * finalScaleY)) /
                              2
                          )
                        )
                      : el.cornerRadius,
        ...
        return new Promise((resolve) => {
          ...
          reader.onload = (e) => {
            try {
              ...
              resolve({
                success: true,
                importedCount: importedElements.length,
              });
            } catch (error) {
      ...
      alignToArtboardRight: (id) => {
        ...
        set((state) => ({
          ...
          elements: state.elements.map((el) =>
            el.id === id
              ? { ...el, x: artboardDimensions.width - el.width }
              : el
          ),
        }));
      },
